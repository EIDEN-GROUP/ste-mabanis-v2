...
export function SearchPanel({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div
      className={compact ? "bg-transparent" : "bg-card/95 p-5 shadow-elegant backdrop-blur sm:p-7"}
    >
      <div className="flex gap-1 border-b border-line">
        {(["vente", "location"] as const).map((t) => (
          <button
            ...
            className={`rounded-md px-4 py-2.5 text-[0.7rem] tracking-[0.16em] uppercase transition-colors ${
              transaction === t
      ...
      <div
        className={`mt-5 grid gap-3 ${compact ? "sm:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-4"}`}
      >
        <label className="flex flex-col gap-1.5">
          ...
          <select
            ...
            className="h-11 rounded-md border border-line bg-background px-3 text-sm outline-none focus:border-gold"
          >
